...
CREATE TABLE
  Bookings (
    booking_id INTEGER UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    user_id INTEGER UNSIGNED NOT NULL,
    offering_id INTEGER UNSIGNED NOT NULL,
    ...
    status ENUM ('pending', 'booked', 'stayed', 'cancelled') NOT NULL DEFAULT 'pending',
    placed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    CONSTRAINT FOREIGN KEY (user_id) REFERENCES Users (user_id) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FOREIGN KEY (offering_id) REFERENCES Offerings (offering_id) ON DELETE CASCADE ON UPDATE CASCADE
  );
